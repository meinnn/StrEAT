...
export const searchOrderList = async (payload: {
  storeId: string
  body: {
    startDate: string
    endDate: string
...
export const fetchTotalWaitng = async () => {
  const storeId = 60
  const response = await fetch(
...
export const postOrderComplete = async (ordersId: string) => {
  const response = await fetch(
...
export const fetchProcessingMyOrderList = async () => {
  const response = await fetch(
...
export const fetchOrderDetail = async (payload: string) => {
  const response = await fetch(
